...
```{r, echo = FALSE}
load("data/RData/iso_data_sub.RData")
...
tmp <- lapply(plts, function(z) {
  ...
  for (iso in z) {
    ...
    xsubd <- seq(0,1, length.out = 8)^2 * diff(range(points[, 1])) + 
      min(points[, 1]) 
    points(bound_points(points, "lower"), type= "l", col = "red")#377EB880")
    points(bound_points2(points, "lower", xsubd = xsubd), 
           type ="l", col = "#377EB880")
...
tmp <- lapply(plts, function(z) {
  ...
  for (iso in z) {
    ...
    s <- which(points[, 1] <= min(points[, 1]) + 0.1 * diff(range(points[, 1])))
    if((nr <- nrow(points)) < 500)
      s <- seq_len(nr)
...
Here I define some helper functions to get the slopes and intercepts from the
bound points.
```{r}
...
plot(points)
res <- combine(bpl, bpu)
addlines(res[, c(2, 3, 4)])
addlines(res[, c(2, 5, 6)])
...
write.table(hmdb_subst, file = paste0(dr_txt, "hmdb_subst_all-neg.txt"),
            sep = "\t", row.names = TRUE)
```
